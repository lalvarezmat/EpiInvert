...
We assume that a festive or anomalous day is any day where we know "a priori" 
that the registered number of cases is biased. Typically, in those days, 
we observe a sharp decrease in the number of registered incidence that is 
compensated by increased incidence numbers in the next few days. We correct such
bias by redistributing the number of cases in the festive day and the next 2 days. 

On top of the festive day bias, there is an strong administrative weekly bias 
introduced by the way the countries registered the new cases each day of the week. 
We correct this weekly bias using 7-day quasi-periodic multiplicative correction
...
Once the festive day and weekly biases are corrected in the incidence curve we 
model the diference between the incidence curve and its expected value using 
the renewal equation by: 
...
In fact, roughly speaking, we could say that the proposed variational model is 
...
We compute experimentally, the power a in the equation (5)  by linear 
...
In [Biology, 2022](https://www.mdpi.com/2079-7737/11/4/540), we show, 
...
user can also provides a non-parametric serial interval given by a numeric vector. 
...
We load stored data on COVID-19 daily incidence up to 2022-05-05 for 
...
### Example 1
We execute EpiInvert using Germany data. The first parameter is a numeric vector 
...
there are more days available. Rt_CI95 represents the radius of an empiric  95% 
...
* power_a : the power a in equation (8). Notice that this value strongly depends on the size of the incidence curve used by the EpiInvert estimation. As we shall see later, this size is an EpiInvert parameter. The estimated value of a only has sense in the case of a large incidence sequence. 
...
EpiInvert execution for France using 365 days in the past. If your are not 
...
Plot of the incidence between "2021-12-15" and "2022-01-15". We observe that 
the festive days bias correction only modify the original incidence in the 
festive days and the following 2 days. 

```{r fig2, fig.width = 7, fig.height = 4, fig.align = "center"}
 plot(res,"incid","2021-12-15","2022-01-15")
